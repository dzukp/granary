...
class BaseOwenMx210(BaseModule):
    def __init__(self, tags, ip, port=502, slave=1, timeout=0.05):
        ...
        self.logger = logger.getChild(
            f'{type(self).__name__}_{ip.replace(".", "")}_{port}'
        )
        self.mb = None
        self.init()
...
class OwenAiMv210(BaseOwenMx210):
    ...
    def process(self):
        ...
        f_data = [
            struct.unpack('>f', struct.pack('>HH', res[i * 3 + 1], res[i * 3]))[0]
            for i in range(0, 8)
        ]
        for tag in self.tags:
            tag.value = (
...
class OwenAoMu210(BaseOwenMx210):
    ...
    def process(self):
        ...
        for tag in self.tags:
            data[tag.addr - 1] = (
                tag.filter.apply(tag.value) if tag.filter else tag.value
            )
        self.mb.execute(
            slave=self.slave,
            function_code=cst.WRITE_MULTIPLE_REGISTERS,
