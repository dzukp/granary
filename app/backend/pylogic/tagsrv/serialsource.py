...
class SerialSource(object):
    ...
    def __init__(
        ...
        try:
            ...
            self.logger.error(
                f'Attempt create serial port failed. Bad parameters: {port} {baudrate} {bytesize}-{parity}-{stopbits}'
            )
        except Exception:
            self.logger.exception(
                f'Attempt crete serial port failed. {port} {baudrate} {bytesize}-{parity}-{stopbits}'
            )
        finally:
    ...
    def write(self, data):
        self.logger.debug(
            f'Serial {self.name} write: hex({binascii.b2a_hex(data)}) ascii({data})'
        )
    ...
    def read(self, required_bytes=0, timeout=0.2):
        ...
        while True:
            ...
            time.sleep(0.001)
        self.logger.debug(
            f'Serial {self.name} read: hex({binascii.b2a_hex(data)}) ascii({data})'
        )
        return data
